...
public class DataParam {
	...
	public DataParam(String value, DataType type, Action action) {
		...
		}else if(type==DataType.ext_asset){
			if(action==action.extransfer){
				this.value="0.01 2,DCCY@eosio.token";
			}
		}
	...
	public byte[] seria() {
		...
		}else if(this.type==DataType.ext_asset){
			return ByteUtils.writerExtAsset(this.value);
		}
		else {
